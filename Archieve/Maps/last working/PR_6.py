...
# This is the gs12083_windmil_export_09_15_2025.mpt file, or any .mpt file
# extracted from the ZIP that windmilfixer.py normally reads from the GIS server.
...
def main():
    ...
    Orchestrate the full pipeline:
        load_mpt_network → load_data → aggregate_data → build_html → write file
    ...
    Example:
        python3 generate_map.py                              # use all defaults
        python3 generate_map.py my_data.xlsx                 # custom input
        python3 generate_map.py my_data.xlsx report.html     # custom in+out
        python3 generate_map.py d.xlsx out.html network.mpt  # with GIS network

    DEBUG:
        Wrap any step in a try/except to isolate failures, e.g.:
            try:
        ...
        aggregate_data() to verify summaries look correct before serialisation.
    """
    ...
    # ── STEP 1: Load the GIS network polylines from the MPT file ────────────
    # This is done first because it's the most time-consuming step (~30 seconds
    # for the full 149k-span file) and failing here doesn't affect the Excel data.
    # If MPT_PATH is None or the file doesn't exist, returns [] and the map
    # renders without network lines (the toggle will show "no MPT" in the UI).
    network_polylines = load_mpt_network(mpt_path, max_spans=MAX_SPANS)

    # ── STEP 2: Load and validate all data from the workbook ────────────────
    outage, flagged_outages, clicks, flagged_clicks = load_data(path)

    # ── STEP 3: Aggregate to per-substation and per-recloser summaries ───────
    sub_agg, rep_agg = aggregate_data(outage, clicks)

    # ── STEP 4: Build the complete HTML string ───────────────────────────────
    html = build_html(sub_agg, rep_agg, flagged_outages, flagged_clicks,
                      network_polylines=network_polylines)

    # ── STEP 5: Write the HTML to disk ───────────────────────────────────────
