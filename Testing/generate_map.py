...
def aggregate_data(outage: pd.DataFrame, clicks: pd.DataFrame,
    ...
    """
    Produce one summary record per substation and per physical OCDEV for the map.

    CORE PRINCIPLE:
        The STD file is the authoritative source of WHERE devices are located.
        Excel data (outage + clicks) tells us WHAT HAPPENED at those locations.
        We iterate the STD OCDEVs first, then join outage data onto them by name.

    SUBSTATION AGGREGATION:
        Source: outage Excel grouped by (Sub, District).
        Coordinates: STD type-9 SOURCE by Sub name -> STD type-10/6 by Next Pro Dvc
                     -> MD5 district-hash fallback if no STD match.

    OCDEV (RECLOSER/FUSE) AGGREGATION:
        Source: every type-10 element in the STD — these are the physical devices.
        Coordinates: STD XL/YL -> lcc_to_latlon(). Always real. Always accurate.
        Outage data: joined by OCDEV name == outage "Next Pro Dvc".
                     Devices with no outage match are still plotted (grey / no-data).

    CLICK-COUNT (RepId) SUMMARY:
        RepId has no STD name match yet so these remain in a separate table for
        the sidebar panel. Once a RepId->OCDEV name lookup is available this can
        be joined directly onto ocdev_agg.

    RETURNS:
        sub_agg   -- per-substation DataFrame
        ocdev_agg -- per-STD-OCDEV DataFrame (real coordinates, optional outage data)
        rep_agg   -- per-RepId DataFrame (sidebar only, coords still approximated)
    """
    ...
    sub_agg = (
        ...
        .agg(
            ...
            next_pro_dvc=("Next Pro Dvc",
                          lambda x: x.dropna().mode().iloc[0] if len(x.dropna()) else ""),
        )
        .reset_index()
    )

    std_hit = npd_hit = approx = 0
    ...
    for _, row in sub_agg.iterrows():
        ...
        if el and el["etype"] == STD_TYPE_SOURCE:
            ...
            coord_sources.append("STD-source"); std_hit += 1; continue

        el = std_elements.get(npd_name)
        if el and el["etype"] in (STD_TYPE_OCDEV, STD_TYPE_SWITCH):
            lats.append(el["lat"]); lons.append(el["lon"])
            coord_sources.append("STD-device"); npd_hit += 1; continue

        lat, lon = district_offset(sub_name, str(row["District"]))
        lats.append(lat); lons.append(lon)
        coord_sources.append("approx"); approx += 1

    sub_agg["lat"]          = lats
    sub_agg["lon"]          = lons
    sub_agg["coord_source"] = coord_sources

    print(f"  Substations: {len(sub_agg)} total")
    print(f"    STD source match:  {std_hit}")
    print(f"    STD device match:  {npd_hit}")
    print(f"    MD5 fallback:      {approx}")

    # ── B) OCDEV LAYER — STD is the source, outage data joins onto it ─────
    # Build per-device outage summary keyed by Next Pro Dvc
    npd_summary = (
        outage.groupby("Next Pro Dvc", dropna=True)
        .agg(
            outage_count=("Outage", "count"),
            customers_affected=("# Out", "sum"),
            total_customer_minutes=("Customer Minutes", "sum"),
        ...
        .reset_index()
        .rename(columns={"Next Pro Dvc": "name"})
    )
    npd_lookup = {str(r["name"]): r for _, r in npd_summary.iterrows()}

    # One row per physical OCDEV in the STD
    ocdev_rows = []
    for name, el in std_elements.items():
        if el["etype"] != STD_TYPE_OCDEV:
            continue
        npd = npd_lookup.get(name)
        ocdev_rows.append({
            "name":               name,
            "lat":                el["lat"],
            "lon":                el["lon"],
            "phase":              el.get("phase", ""),
            "closed_a":           el.get("closed_a", ""),
            "closed_b":           el.get("closed_b", ""),
            "closed_c":           el.get("closed_c", ""),
            "feeder":             el.get("feeder", ""),
            "desc":               el.get("desc_a", ""),
            "outage_count":       int(npd["outage_count"])           if npd is not None else 0,
            "customers_affected": int(npd["customers_affected"])     if npd is not None else 0,
            "customer_minutes":   int(npd["total_customer_minutes"]) if npd is not None else 0,
            "causes":             dict(npd["causes"])                if npd is not None else {},
            "has_outage_data":    npd is not None,
        })

    ocdev_agg = pd.DataFrame(ocdev_rows) if ocdev_rows else pd.DataFrame()

    if len(ocdev_agg):
        omax = ocdev_agg["outage_count"].max()
        ocdev_agg["severity_score"] = ocdev_agg["outage_count"] / max(omax, 1)

    matched = int(ocdev_agg["has_outage_data"].sum()) if len(ocdev_agg) else 0
    print(f"  OCDEVs (from STD): {len(ocdev_agg)} at real coordinates")
    print(f"    With outage data: {matched}")
    print(f"    No outage data (plotted grey): {len(ocdev_agg) - matched}")

    # ── C) CLICK-COUNT SUMMARY BY RepId (sidebar only) ────────────────────
    rep_agg = (
    ...
    print(f"  Click RepIds: {len(rep_agg)} (sidebar only — no STD location yet)")

    return sub_agg, ocdev_agg, rep_agg
...
def build_html(sub_agg: pd.DataFrame, ocdev_agg: pd.DataFrame,
               rep_agg: pd.DataFrame,
    ...
    # ── STEP 1: Build the OCDEV (recloser/fuse) data list ───────────────────
    # Source: ocdev_agg — one row per physical device in the STD file.
    # Coordinates come from the STD XL/YL columns (real surveyed locations).
    # Colour: green-grey for no outage data, green->red scale for outage count.
    ocdevs_js = []
    for _, r in ocdev_agg.iterrows():
        score = float(r.get("severity_score", 0))
        has_data = bool(r.get("has_outage_data", False))
        color = click_color(score) if has_data else "#666666"
        ocdevs_js.append({
            "name":              safe_str(r["name"]),
            "lat":               float(r["lat"]),
            "lon":               float(r["lon"]),
            "phase":             safe_str(r.get("phase", "")),
            "closedA":           safe_str(r.get("closed_a", "")),
            "closedB":           safe_str(r.get("closed_b", "")),
            "closedC":           safe_str(r.get("closed_c", "")),
            "feeder":            safe_str(r.get("feeder", "")),
            "desc":              safe_str(r.get("desc", "")),
            "outageCount":       int(r.get("outage_count", 0)),
            "customersAffected": int(r.get("customers_affected", 0)),
            "customerMinutes":   int(r.get("customer_minutes", 0)),
            "causes":            r.get("causes", {}),
            "hasOutageData":     has_data,
            "severityScore":     score,
            "color":             color,
        })

    # ── STEP 1b: Build click-count sidebar list (RepId only, no map dots) ───
    # RepId has no STD coordinate match yet, so these are shown in the sidebar
    # only — not plotted as dots on the map.
    clicks_js = []
    ...
    for _, r in flagged_outages.iterrows():
        flagged_out_js.append({
            "outage":  safe_str(r.get("Outage")),
            "missing": r["_missing_fields"],
            "timeOff": safe_str(r.get("Time Off")),
    ...
    # ── STEP 4: Serialise all lists to JSON ─────────────────────────────────
    ocdevs_json        = json.dumps(ocdevs_js)
    clicks_json        = json.dumps(clicks_js)
...
<header>
  ...
  <div class="header-stats">
    <div class="stat-chip">DEVICES <span>{len(ocdev_agg)}</span></div>
    ...
    <div id="tab-controls" class="tab-pane active">
      ...
      <div class="control-section">
        ...
        <div class="toggle-row">
          <label for="tog-reclosers">Devices (STD)</label>
          <label class="toggle">
            <input type="checkbox" id="tog-reclosers" checked onchange="toggleLayer('ocdevs', this.checked)"/>
...
// OCDEVS — one object per physical overcurrent device from the STD file.
// Coordinates are REAL surveyed field locations (STD XL/YL -> lcc_to_latlon).
// Outage data is joined by element name where available.
// To inspect: console.log('Devices loaded:', OCDEVS.length, OCDEVS[0])
const OCDEVS      = {ocdevs_json};

// CLICKS_DATA — click-count summary per RepId (SCADA numeric ID).
// No map coordinates — RepId has no STD name match yet.
// Shown in the sidebar Reclosers tab for reference only.
const CLICKS_DATA = {clicks_json};

const SUBSTATIONS = {substations_json}; // One object per (Sub, District) pair
const FLAGGED_OUT = {flagged_out_json}; // Outage rows missing critical data
const FLAGGED_CLK = {flagged_click_json}; // Click-count rows missing critical data
...
const layers = {{
  ocdevs:     L.layerGroup().addTo(map),  // Physical OCDEV devices from STD
  substations:L.layerGroup().addTo(map),  // Substation markers
 ...
 */
function buildOcdevMarkers() {{
  // Clear any existing markers before rebuilding
  layers.ocdevs.clearLayers();

  OCDEVS.forEach(d => {{
    const [rv, gv, bv] = hexToRgb(d.color);

    // Devices with outage data get a coloured fill (green->red by severity).
    // Devices with no outage data get a grey fill with reduced opacity.
    const marker = L.circleMarker([d.lat, d.lon], {{
      radius:      dotRadius,
      fillColor:   d.color,
      color:       d.hasOutageData
                     ? `rgba(${{rv}},${{gv}},${{bv}},0.5)`
                     : 'rgba(120,120,120,0.4)',
      weight:      d.hasOutageData && d.severityScore > 0.7 ? 2 : 1,
      fillOpacity: d.hasOutageData ? 0.85 : 0.4,
    }});

    marker.bindPopup(buildOcdevPopup(d), {{ maxWidth: 320 }});
    marker.addTo(layers.ocdevs);
  }});
}}

/**
 * buildOcdevPopup(d)
 * Build the HTML popup for an OCDEV (overcurrent device) marker.
 * Shows real field data from the STD: name, phase, open/closed status, feeder.
 * If outage data was matched by name, shows outage count and top causes.
 *
 * @param {{object}} d - An OCDEV object from the OCDEVS array
 */
function buildOcdevPopup(d) {{
  // Open/closed status from the STD — 1=closed, 0=open, 2=bypassed
  const statusA = d.closedA === '1' ? '✓ Closed' : d.closedA === '0' ? '✗ Open' : d.closedA === '2' ? '~ Bypass' : '—';
  const statusB = d.closedB === '1' ? '✓ Closed' : d.closedB === '0' ? '✗ Open' : d.closedB === '2' ? '~ Bypass' : '—';
  const statusC = d.closedC === '1' ? '✓ Closed' : d.closedC === '0' ? '✗ Open' : d.closedC === '2' ? '~ Bypass' : '—';

  const phaseLabel = {{
    '1':'A', '2':'B', '3':'C', '4':'AB', '5':'AC', '6':'BC', '7':'ABC'
  }}[d.phase] || d.phase;

  const severityBadge = !d.hasOutageData
    ? '<span class="badge" style="background:#444;color:#aaa">NO DATA</span>'
    : d.severityScore > 0.7
      ? '<span class="badge badge-red">HIGH</span>'
      : d.severityScore > 0.35
        ? '<span class="badge badge-yellow">MED</span>'
        : '<span class="badge badge-green">LOW</span>';

  const causeRows = Object.entries(d.causes || {{}})
    .map(([k, v]) => `
      <div class="popup-row">
  ...
  return `
    ...
    <div class="popup-row">
      ...
      <span class="popup-val">${{severityBadge}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Location</span>
      <span class="popup-val" style="font-size:10px;color:#00ff99">✓ GIS (STD)</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Phase</span>
      <span class="popup-val">${{phaseLabel}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Feeder</span>
      <span class="popup-val">${{d.feeder || '—'}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Description</span>
      <span class="popup-val">${{d.desc || '—'}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Ph A Status</span>
      <span class="popup-val">${{statusA}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Ph B Status</span>
      <span class="popup-val">${{statusB}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Ph C Status</span>
      <span class="popup-val">${{statusC}}</span>
    </div>
    ${{d.hasOutageData ? `
    <div class="popup-row">
      <span class="popup-key">Outages</span>
      <span class="popup-val">${{d.outageCount}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Customers Affected</span>
      <span class="popup-val">${{d.customersAffected}}</span>
    </div>
    <div class="popup-row">
      <span class="popup-key">Customer·Minutes</span>
      <span class="popup-val">${{d.customerMinutes.toLocaleString()}}</span>
    </div>` : ''}}
    ${{causeRows ? `
    <div class="popup-advanced" id="adv-${{safeId}}">
      <div style="font-size:10px;color:var(--muted);margin-bottom:4px;font-family:'Share Tech Mono',monospace">TOP CAUSES</div>
      ${{causeRows}}
    </div>
    <button class="detail-toggle" onclick="toggleAdv('${{safeId}}', this)">▼ Show Causes</button>
    ` : ''}}
  `;
}}
...
function buildClickHeatmap() {{
  ...
  // Use OCDEV outage counts for the heatmap — these are real field locations.
  // Only include devices that have outage data.
  const pts = OCDEVS
    .filter(d => d.hasOutageData && d.outageCount > 0)
    .map(d => [d.lat, d.lon, d.outageCount]);
...
function filterMinClicks(v) {{
  minClicks = v;
  document.getElementById('min-click-val').textContent = v;
  buildOcdevMarkers();
  buildClickHeatmap();
}}
...
function buildRecloserList() {{
  // Section 1: Physical OCDEV devices from STD — sorted by outage count
  const sortedDevices = [...OCDEVS].sort((a, b) => b.outageCount - a.outageCount);

  let html = `<div class="control-label" style="margin-bottom:6px">
    FIELD DEVICES — STD (${{OCDEVS.length}})
    <span style="font-size:9px;color:var(--muted);margin-left:4px">real GIS locations</span>
  </div>`;

  html += sortedDevices.map(d => {{
    const badge = !d.hasOutageData
      ? '<span class="badge" style="background:#333;color:#888;font-size:9px">NO DATA</span>'
      : d.severityScore > 0.7
        ? '<span class="badge badge-red">HIGH</span>'
        : d.severityScore > 0.35
          ? '<span class="badge badge-yellow">MED</span>'
          : '<span class="badge badge-green">LOW</span>';
    return `
      <div class="info-item" onclick="flyTo(${{d.lat}}, ${{d.lon}}, 16)">
        <div class="iname">${{badge}} ${{d.name}}</div>
        <div class="imeta">
          ${{d.feeder ? d.feeder + ' | ' : ''}}
          ${{d.hasOutageData ? d.outageCount + ' outages' : 'no outage data'}}
          ${{d.customersAffected ? ' | ' + d.customersAffected + ' cust.' : ''}}
        </div>
      </div>`;
  }}).join('');

  // Section 2: Click-count data by RepId (no map location)
  if (CLICKS_DATA.length) {{
    const sortedClicks = [...CLICKS_DATA].sort((a, b) => b.totalClicks - a.totalClicks);
    html += `<div class="control-label" style="margin:12px 0 6px">
      CLICK DATA — RepId (${{CLICKS_DATA.length}})
      <span style="font-size:9px;color:var(--muted);margin-left:4px">no GIS location yet</span>
    </div>`;
    html += sortedClicks.map(r => {{
      const badge = r.score > 0.7
        ...
        : r.score > 0.35
          ...
          : '<span class="badge badge-green">LOW</span>';
      return `
        <div class="info-item" style="opacity:0.65;cursor:default">
          <div class="iname">${{badge}} RepId ${{r.repId}}</div>
          <div class="imeta">Clicks: ${{r.totalClicks}} | Windows: ${{r.eventCount}}</div>
        </div>`;
    }}).join('');
  }}

  document.getElementById('recloser-list').innerHTML = html;
}}
...
buildOcdevMarkers();      // Step 2: Draw OCDEV dots at real STD coordinates
buildSubstationMarkers(); // Step 3: Draw substation rings
...
buildRecloserList();      // Step 6: Populate Devices/Reclosers sidebar tab
...
def main():
    ...
    # ── STEP 4: Aggregate — STD coords for OCDEVs and substations ────────────
    sub_agg, ocdev_agg, rep_agg = aggregate_data(
        outage, clicks, std_elements=std_elements
    )

    # ── STEP 5: Build the complete HTML string ────────────────────────────────
    html = build_html(sub_agg, ocdev_agg, rep_agg,
                      flagged_outages, flagged_clicks,
